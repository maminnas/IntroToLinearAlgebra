...
public class VectorStuff : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < size; i++)//lines.Length
        {
            ...
            newLine[i].SetWidth(0.01f, 0.01f);
            newLine[i].SetPosition(0, new Vector3(0,0,1.5f));
            newLine[i].SetPosition(1, new Vector3(0, 0, 1.5f));
    ...
    public void DrawArrow(GameObject v, Vector3 startPoint, Vector3 endPoint)
    {
        ...
        //v.transform.position = new Vector3((startPoint.x + endPoint.x) / 2, (startPoint.y + endPoint.y) / 2, (startPoint.z + endPoint.z) / 2); //Vector3.Lerp(startPoint, endPoint, 0.5f); 
